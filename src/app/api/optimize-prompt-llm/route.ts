/**
 * LLM-Based Prompt Optimizer API Route
 * 
 * Uses Gemini 2.0 Flash (FREE) for dynamic, iterative prompt optimization
 * 
...
// Non-streaming optimization takes ~20-30s per round, ~90s for 3 rounds
// 5 minutes provides buffer for slower responses
 ...
 * This is the "secret sauce" - LLM optimizes its own prompts iteratively
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (let round = 1; round <= optimizationRounds; round++) {
      ...
      try {
        // Generate meta-prompt for this round
        ...
        let jsonMatch = response.match(/SCORE_START\s*([\s\S]*?)\s*SCORE_END/);
        ...
        roundData.weaknesses = parsed.weaknesses;
        roundData.improvements = parsed.improvements;
        roundData.optimizedPrompt = parsed.optimizedPrompt;
        roundData.overallScore = overallScore;
        roundData.status = 'complete';
        ...
        Sentry.captureException(error, {
          tags: { round, endpoint: 'optimize-prompt-llm' },
          extra: { currentPrompt, optimizationRounds }
        });

        roundData.status = 'complete';
        roundData.optimizedPrompt = currentPrompt; // Keep previous
      }
    ...
    return NextResponse.json({
      ...
      original: prompt,
      rounds: results,
      finalPrompt: results[results.length - 1]?.optimizedPrompt || prompt,
      finalScore: results[results.length - 1]?.overallScore || 0,
    });
    ...
    // Capture error in Sentry
    Sentry.captureException(error, {
      tags: { error_type: 'optimization', endpoint: 'optimize-prompt-llm' }
    });
